...
impl CalledSample {
    ...
    pub fn likelihood(&self) -> &SampleLikelihood {
        &self.likelihood
    }
}
...
impl MultiallelicCaller {
    ...
    pub fn call(&self, site: &LikelihoodSite) -> Result<CalledSite> {
        ...
        {
            return Err(CallError::UnsupportedCallerPloidy);
        }
        ...
        let log_prior = adjusted_log_prior(
            self.config.mutation_rate,
            site.samples().len().saturating_mul(2),
        );
        let selection = select_alleles(&likelihoods, &quality_sums, log_prior);
        let unseen = site
            .alternates()
            .iter()
...
fn select_alleles(
    ...
    for first in 0..allele_count {
        ...
        for second in 0..first {
            ...
            let mut supported = false;
            for likelihoods in sample_likelihoods {
                let value = frequencies[0].powi(2) * likelihoods[genotype_index(first, first)]
...
fn call_sample(
    ...
    let likelihood = trimmed_likelihood(sample, retained, true)?;
    ...
    let genotype_count = retained.len() * (retained.len() + 1) / 2;
    let mut probabilities = vec![0.0f32; genotype_count];
    let mut best = 0.0;
    let mut genotype = [0usize, 0usize];
    for &right in retained {
        let new_right = old_to_new[right].unwrap();
        let homozygous = likelihoods[genotype_index(right, right)] * quality_sums[right].powi(2);
        probabilities[genotype_index(new_right, new_right)] = homozygous as f32;
        if best < homozygous {
            best = homozygous;
            genotype = [new_right, new_right];
        }
...
fn call_reference_sample(
    ...
    Ok(CalledSample {
        genotype: supported.then(|| vec![0, 0].into_boxed_slice()),
        genotype_quality: 0,
        genotype_probabilities: None,
        likelihood: trimmed_likelihood(sample, &[0], false)?,
    })
}

fn trimmed_likelihood(
    sample: &SampleLikelihood,
    retained: &[usize],
    retain_likelihoods: bool,
) -> Result<SampleLikelihood> {
    ...
    let phred_likelihoods = retain_likelihoods
        .then(|| {
            sample.phred_likelihoods().map(|values| {
                let mut trimmed = Vec::with_capacity(retained.len() * (retained.len() + 1) / 2);
        ...
        .flatten();
    SampleLikelihood::new(Ploidy::new(2).unwrap(), phred_likelihoods, evidence)
}
...
mod tests {
    ...
    fn matches_bcftools_1_24_two_sample_multiallelic_call() {
        ...
        assert_eq!(
            called.samples()[0].likelihood().phred_likelihoods(),
            Some(&[0, 3, 40][..])
        );
        assert_eq!(
            called.samples()[1].likelihood().phred_likelihoods(),
            Some(&[40, 3, 0][..])
        );
        let expected = [
    ...
    fn rejects_non_diploid_likelihoods_and_invalid_prior() {
        ...
        assert_eq!(
            ...
            Err(CallError::UnsupportedCallerPloidy)
        );
    }
    ...
    fn matches_bcftools_1_24_reference_call() {
        ...
        assert_eq!(called.samples()[0].likelihood().phred_likelihoods(), None);
        assert_eq!(
            called.samples()[0]
                .likelihood()
                .evidence()
